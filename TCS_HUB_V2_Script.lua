-- ⚽ TCS HUB V2 Delta - Reach Avançado (Drible e Condução Suave)
...
-- Config
local CONFIG = {
    reach = 10,                -- Alcance da perna / raio da esfera
    maxPush = 50,              -- Quanto a bola pode ser empurrada
    showReachSphere = true,     -- Mostrar esfera azul
    autoSecondTouch = true,     -- Ativa toque automático
    scanCooldown = 2,           -- Atualiza bolas a cada X segundos
    ballNames = { "TPS", "ESA", "MRS", "PRS", "MPS" }  -- Nomes das bolas
}
...
local function updateReachSphere()
    ...
    if not reachSphere then
        ...
        reachSphere.Color = Color3.fromRGB(0,120,255)
...
-- Função principal de conduzir / tocar bola
    ...
    for _, ball in ipairs(balls) do
        for _, leg in ipairs(legParts) do
            ...
            if distance <= CONFIG.reach then
                pcall(function()
                    local direction = (ball.Position - leg.Position).Unit
                    local push = math.clamp(CONFIG.maxPush * (1 - (distance/CONFIG.reach)),0,CONFIG.maxPush)
                    ball.Velocity = direction * push
                end)
                break
            end
        end
...
-- Atualização automática das bolas
task.spawn(function()
    while true do
        refreshBalls(false)
...
print("🔵 TCS HUB V2 Delta carregado! Reach avançado funcional, condução e drible suave ativo!")
